...
见 [人工改写清单](docs/HANDWRITTEN_PLAN.md)：40/93 个核心设计单元（43.0%）已标出。每个相关文件只使用一组 `##以下为手写` 和 `##手写区域结束`，中间整段均属于手写区域。其余区域默认 AI 生成，底层存储不纳入手写范围。验收结果见 [自检报告](docs/SELF_CHECK.md)。测试使用临时数据目录，不依赖迁移备份目录。
